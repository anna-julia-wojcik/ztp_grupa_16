...
for year in years:
    try:
        dataframes[year] = download_gios_archive(year, gios_url_ids[year], gios_pm25_file[year])
...
try:
    ...
    for old_codes, new_code in new_codes_raw.items():
      # jeśli więcej niż jeden kod stary to rozdziel po przecinku na wiele kluczy
        for code in [c.strip() for c in str(old_codes).split(',')]:
            new_codes[code] = new_code
    print("Utworzono bazę danych nowych kodów stacji")
except Exception as e:
...
multi_index = pd.MultiIndex.from_tuples([(code_city.get(code, "Brak"), code) for code in common_stations], names=["Miejscowość", "Kod stacji"])
